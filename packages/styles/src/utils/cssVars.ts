...
export type DefaultPrefix = 'nx-';
/** 默认情况下，生成 CSS 变量时增加的前缀 */
export const DEFAULT_PREFIX: DefaultPrefix = 'nx-';
...
export function generateCssVars<
  ...
  K extends keyof T = keyof T,
  P extends string = DefaultPrefix,
>(origin: T, options?: GenerateCssVarsOptions<K, P>): CssVarObject<T, K, P> {
  ...
  return result as CssVarObject<T, K, P>;
}
...
export function getCssVar<T extends Record<string, any> = Record<string, any>>(
  name: keyof T,
...
export function cssVarToRgba<T extends Record<string, any> = Record<string, any>>(
  name: keyof T,
  alpha: number = 1,
  prefix: string = DEFAULT_PREFIX,
) {
