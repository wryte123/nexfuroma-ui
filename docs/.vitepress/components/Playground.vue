...
import '@vue/repl/style.css?inline';
...
async function fetchUiVersions() {
  const res = await fetch('https://data.jsdelivr.com/v1/package/npm/@nexfuromaui/ui');
  const { versions } = (await res.json()) as { versions: string[] };
...
function setUiVersion(version: string) {
  ...
  store.setImportMap({
    imports: {
      '@nexfuromaui/ui': `https://fastly.jsdelivr.net/npm/@nexfuromaui/ui@${version}/dist/nexfuromaui-ui.full.min.mjs`,
    },
  });
  // 加载组件库的全量 css
  previewOptions.headHTML = `<link rel="stylesheet" href="https://fastly.jsdelivr.net/npm/@nexfuromaui/ui@${version}/dist/style/index.css">`;
}
</script>
...
<template>
  <div v-if="isMounted">
    <Repl
      :store="store" :editor="Monaco" :auto-resize="true" :clear-console="false"
      :preview-options="previewOptions"
    />

    <Teleport to=".VPNavBarSearch">
      <div class="flex items-center text-14px">
        <label class="playground-label">NexfuromaUI: </label>
...
.playground-select {
  ...
  margin-left: 8px;
  appearance: auto;
  border: 1px solid rgb(var(--nx-color-bd_base));
}
</style>
